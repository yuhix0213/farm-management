// pages/index.tsx — ダッシュボード（認証後のトップページ）
  ...
  return (
    <>
      ...
      <main style={{ padding:24, fontFamily:'sans-serif', background:'#F0F4F8', minHeight:'calc(100vh - 48px)' }}>
        <h1 style={{ fontSize:20, fontWeight:600, color:'#1D3557', marginBottom:20 }}>
          ダッシュボード
        </h1>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:16 }}>
          {nav.map(n => (
            <div key={n.href} onClick={() => router.push(n.href)} style={{
              background:'#fff', borderRadius:12, border:'0.5px solid #D0D7E0',
